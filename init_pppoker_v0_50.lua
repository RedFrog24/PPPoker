...
--[[ GUI — MageGear-style *Lua* layout only.
     MacroQuest ships ONE ImGui plugin: require('ImGui') is the same for every script.
     We did not copy separate "ImGui files" from MageGear; only the call order (Begin open/draw, Image, Child).
     If backdrop scaling looks wrong here AND with AB test water.png, the issue is this window/callback context or MQ texture+Image, not "which script's GUI code" copied. ]]
...
--- Set true to load the same water.png MageGear uses (mq.luaDir/magegear/water.png). Reload script after changing.
--- If water FILLS the PPPoker window but rooster does not → likely that PNG/texture; if both behave the same → not the rooster file alone.
local PPPOKER_AB_TEST_MAGEGEAR_WATER = false

--- Set true once to print ContentRegionAvail vs window content rect (one message per /lua run).
local PPPOKER_GUI_LOG_SIZES_ONCE = false

--- "image" = show PNG (MageGear-style; may sit small top-left on some MQ). "none" = no backdrop art. "drawlist" = AddImage then fallback to Image if needed.
local PPPOKER_BACKDROP_MODE = "image"

--- Same folder as init.lua under mq.luaDir/pppoker/ (when not AB test).
local PPPOKER_BG_FILENAME = "pppoker_bg_roosters_playing_cards.png"
local pppokerBgTexState = nil
local pppokerBgResolvedPath = nil
local pppokerGuiDbgPrinted = false
local pppokerBackdropMissingLogged = false

local function getPppokerBackdropPath()
    local dir = (mq.luaDir or "."):gsub("[\\/]+$", "")
    if PPPOKER_AB_TEST_MAGEGEAR_WATER then
        return dir .. "/magegear/water.png"
    end
    return dir .. "/pppoker/" .. PPPOKER_BG_FILENAME
end

local function getPppokerBgTexture()
    local path = getPppokerBackdropPath()
    if pppokerBgResolvedPath ~= path then
        pppokerBgTexState = nil
        pppokerBgResolvedPath = path
    end
...
local function pppokerTwoNums(a, b)
...
--- Screen-space rect covering full window content region (for draw-list backdrop).
local function pppokerContentRectScreen()
    local mina, minb = imgui.GetWindowContentRegionMin()
    local maxa, maxb = imgui.GetWindowContentRegionMax()
    local minx, miny = pppokerTwoNums(mina, minb)
    local maxx, maxy = pppokerTwoNums(maxa, maxb)
    local ax = math.max(1, maxx - minx)
    local ay = math.max(1, maxy - miny)
    imgui.SetCursorPos(minx, miny)
    local sa, sb = imgui.GetCursorScreenPos()
    local sx, sy = pppokerTwoNums(sa, sb)
    return sx, sy, ax, ay, minx, miny
end

--- MageGearGUI pattern: Begin returns open, draw; optional backdrop; controls in BeginChild.
local function DrawGUI()
    imgui.SetNextWindowSize(420, 260, ImGuiCond.FirstUseEver)
    -- Literal `true` every frame resets p_open; pass prior gui.open so X can set false and stick until script ends.
    local open, draw = imgui.Begin("PPPoker v" .. VERSION .. "###PPPoker", gui.open)
    gui.open = open
    ...
    if draw then
        local ok, err = pcall(function()
            ...
            local mode = string.lower(tostring(PPPOKER_BACKDROP_MODE or "image"))
            local imageFile = (mode ~= "none") and getPppokerBgTexture() or nil
            if mode ~= "none" and not imageFile and not pppokerBackdropMissingLogged then
                pppokerBackdropMissingLogged = true
                print("[PPPoker] Backdrop PNG not loaded (CreateTexture missing or file not found): " .. getPppokerBackdropPath())
            end
            if PPPOKER_GUI_LOG_SIZES_ONCE and not pppokerGuiDbgPrinted then
                pppokerGuiDbgPrinted = true
                local wwx, wwy = imgui.GetWindowSize()
                local mina, minb = imgui.GetWindowContentRegionMin()
                local maxa, maxb = imgui.GetWindowContentRegionMax()
                print(string.format(
                    "[PPPoker GUI] GetContentRegionAvail=%.1f x %.1f  CursorPos=%.1f,%.1f  WinSize=%.1fx%.1f  ContentMin=(%.1f,%.1f) Max=(%.1f,%.1f)  span=%.1fx%.1f",
                    sizeX, sizeY, cursorX, cursorY, wwx or 0, wwy or 0,
                    mina or 0, minb or 0, maxa or 0, maxb or 0,
                    (maxa or 0) - (mina or 0), (maxb or 0) - (minb or 0)
                ))
            end
            if mode ~= "none" and imageFile ~= nil and imageFile.GetTextureID and imageFile:GetTextureID() then
                local tid = imageFile:GetTextureID()
                if mode == "drawlist" then
                    local sx, sy, ax, ay, minx, miny = pppokerContentRectScreen()
                    local dl = imgui.GetWindowDrawList()
                    local dim = imgui.GetColorU32(ImVec4(0.02, 0.02, 0.05, 0.95))
                    dl:AddRectFilled(ImVec2(sx, sy), ImVec2(sx + ax, sy + ay), dim, 0, 0)
                    local tintU32 = imgui.GetColorU32(ImVec4(1, 1, 1, 0.55))
                    local ti = tonumber(tintU32) or tintU32
                    local dlOk = pcall(function()
                        dl:AddImage(tid, ImVec2(sx, sy), ImVec2(sx + ax, sy + ay), ImVec2(0, 0), ImVec2(1, 1), ti)
                    end)
                    if not dlOk then
                        imgui.SetCursorPos(minx, miny)
                        local imgOk = pcall(function()
                            imgui.Image(tid, ImVec2(sizeX, sizeY), nil, nil, ImVec4(1, 1, 1, 0.55))
                        end)
                        if not imgOk then
                            pcall(function()
                                imgui.Image(tid, ImVec2(sizeX, sizeY))
                            end)
                        end
                    end
                    imgui.SetCursorPos(minx, miny)
                else
                    -- "image" — same as MageGear; may not stretch CreateTexture on some MQ builds.
                    imgui.Indent(5)
                    local tinted = pcall(function()
                        imgui.Image(tid, ImVec2(sizeX, sizeY), nil, nil, ImVec4(1, 1, 1, 0.55))
                    end)
                    if not tinted then
                        imgui.Image(tid, ImVec2(sizeX, sizeY))
            ...
            imgui.PushStyleColor(ImGuiCol.FrameBg, ImVec4(frameBG.x, frameBG.y, frameBG.z, 0.4))
            imgui.PushStyleColor(ImGuiCol.ChildBg, ImVec4(0, 0, 0, 0.7))

            local childFlags = (ImGuiChildFlags and ImGuiChildFlags.Border) or true
            imgui.BeginChild("##pppoker_child", 0, 0, childFlags)
            ...
            imgui.PushStyleColor(ImGuiCol.PlotHistogram, ImVec4(0.2, 0.9, 0.4, 0.9))
            imgui.ProgressBar(p, ImVec2(-1, 18), "")
            imgui.PopStyleColor()
