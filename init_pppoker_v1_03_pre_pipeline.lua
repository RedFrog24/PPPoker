...
local PP = {
    ...
    POK_SOULBINDER_LOC_X = -131.6,
    POK_SOULBINDER_LOC_Y = -94.2,
    POK_SOULBINDER_LOC_Z = -159.0,
    --- Big Slick corner (same triple as legacy Poker.lua `/nav locyxz 19 136 -54`).
    SLICK_QUEST_LOC_X = 19,
    SLICK_QUEST_LOC_Y = 136,
    SLICK_QUEST_LOC_Z = -54,
    GATE_ALT_ACT_ID = 1217,
    GATE_POTION_NAME = "Philter of Major Translocation",
    ZUERIA_SLIDE_BASE = "Zueria Slide",
    ZUERIA_TARGET_MODE = "Nektulos",
    -- Zueria Slide is commonly level-gated around 105 for Nektulos conversions on Live.
    -- Some MQ TLO builds don't expose item.RequiredLevel reliably, so we keep a hard floor.
    ZUERIA_SLIDE_REQUIRED_LEVEL = 105,
    ...
    SPIRITUAL_VIGOR = "Spiritual Vigor",
    COMMEMORATIVE_ITEM_ICON_ID = 5901,
    DRAGITEM_ICON_ATLAS_OFFSET = 500,
    NPC_BIG_SLICK_JONES = { "Big Slick Jones" },
    NPC_BLUFFING_BETTY = { "Bluffing Betty", "Bluffing" },
    NPC_QUINN_OF_QUADS = { "Quinn of Quads", "Quads" },
    NPC_MHRAI_QUEEN_OF_TAILS = { "Mhrai, Queen of Tails", "Queen", "Mhrai" },
    NPC_SOULBINDER_JERA = { "Soulbinder Jera", "Jera" },
...
local function getZueriaSlideName()
    local ok, name = pcall(function() return mq.TLO.FindItem(PP.ZUERIA_SLIDE_BASE).Name() end)
    if not ok or not name or name == "" then return nil end
    return tostring(name)
end

--- Returns true when a Zueria Slide is present and usable at current level.
local function canUseZueriaSlideNow()
    local ok, item = pcall(function() return mq.TLO.FindItem(PP.ZUERIA_SLIDE_BASE) end)
    ...
    pcall(function()
        if item.RequiredLevel then
            req = tonumber(item.RequiredLevel() or 0) or 0
        end
    end)
    local lvl = nil
    pcall(function() lvl = tonumber(mq.TLO.Me.Level() or 0) or 0 end)
    if (not req or req <= 0) and PP.ZUERIA_SLIDE_REQUIRED_LEVEL and PP.ZUERIA_SLIDE_REQUIRED_LEVEL > 0 then
        req = tonumber(PP.ZUERIA_SLIDE_REQUIRED_LEVEL) or 0
    end
    if req and req > 0 and lvl and lvl > 0 and lvl < req then
        return false, req, lvl
    end
    return true, req, lvl
end

local function ensureZueriaMode(targetMode)
    targetMode = targetMode or PP.ZUERIA_TARGET_MODE
...
local function pppokerHasSpiritualVigor()
    local ok, id = pcall(function() return mq.TLO.Me.Buff(PP.SPIRITUAL_VIGOR).ID() end)
    return ok and id and tonumber(id or 0) > 0
end

--- True run-speed only: class SoW/Selo/etc. Spiritual Vigor is HP/stamina — not movement (do not count here).
...
--- ${Me.Invis(1)} — living/normal invis (InvisDisplay-style); use with pppokerMqBool only.
...
local function reportTravelBuffsPreflight()
    ...
    if pppokerHasSpiritualVigor() then
        info("- Spiritual Vigor: active (HP/stamina — not counted as run-speed).")
    end
...
local function preflight()
    ...
    if slideName and slideName:find(PP.ZUERIA_TARGET_MODE, 1, true) then
        info('- Zueria Slide: ' .. slideName .. ' (ready)')
    elseif slideName then
        warn('- Zueria Slide: ' .. slideName .. ' (will convert to ' .. PP.ZUERIA_TARGET_MODE .. ' when needed)')
    else
...
local function navToBigSlickWaypoint()
    mq.cmdf(
        "/squelch /nav locyxz %.1f %.1f %.1f",
        PP.SLICK_QUEST_LOC_X,
        PP.SLICK_QUEST_LOC_Y,
        PP.SLICK_QUEST_LOC_Z
    )
    moving()
...
local function ensurePokBind()
    ...
    mq.cmdf(
        "/squelch /nav locyxz %.1f %.1f %.1f",
        PP.POK_SOULBINDER_LOC_X,
        PP.POK_SOULBINDER_LOC_Y,
        PP.POK_SOULBINDER_LOC_Z
    )
    moving()
    mq.delay(1000)
    targetOrFail(PP.NPC_SOULBINDER_JERA, "Could not target Soulbinder Jera for PoK bind")
    mq.delay(500)
    mq.cmd('/face fast')
    mq.cmd('/say Bind')
    mq.delay(5000)
...
if questComplete then
    setGuiStage(8, "Journal shows all objectives complete — stopping")
    mq.cmd('/popup Quest already completed; stopping run.')
    unpauseRGMercs()
    unpauseCWTNPlugins()
    return
end
setGuiStage(startStage or 1, string.format("Resuming (~route block %d)", startStage or 1))
 
-- West Freeport. Talk to Big Slick for quest.
if startStage <= 1 then
    local needs1, needAt1 = taskNeedsStageBlockOrEarlier(1)
    if needs1 then
        setGuiStage(1, "West Freeport — Big Slick / accept task")
        shouldStop()
        -- Slick's corner is safe — no invis for quest pickup (invis before Tassel's / East FP instead).
        if mq.TLO.Zone.ID() ~= 383 then
           mq.cmd('/squelch /travelto freeportwest')
           zoning(383)
        end
        ...
        mq.delay(500)
        if not waitForNPCToBeWithinDistance(PP.NPC_BIG_SLICK_JONES, 25, 12000) then
            pppokerGuiDebug("Slick not in range yet — re-naving to keyword pocket.")
            navToBigSlickWaypoint()
            mq.delay(1000)
            waitForNPCToBeWithinDistance(PP.NPC_BIG_SLICK_JONES, 25, 12000)
        end
        dismountIfMounted("Targeting Big Slick Jones (quest)")
        mq.delay(400)
        targetOrFail(PP.NPC_BIG_SLICK_JONES, "Could not target Big Slick Jones", 15000)
        mq.delay(1000)
        mq.cmd('/face fast')
        if not getActivePokerTaskIndex() then
        ...
        -- We have completed the Slick handoff step; keep status from sticking at "Getting quest".
        gui.expectingQuestFromRun = false
    else
        info(string.format("Skipping stage 1/8 (West Freeport): task already past this step (at stage %s).", tostring(needAt1)))
        setGuiStage(needAt1 or 1, string.format("Skipped — route ahead of block 1 (~%s)", tostring(needAt1)))
    end
end

if startStage <= 2 then
    local needs2, needAt2 = taskNeedsStageBlockOrEarlier(2)
    local missTassel = hasIncompleteTasselTavernObjective()
    if needs2 or missTassel then
        setGuiStage(2, "Buffing for city bar run (West -> East Freeport)")
        shouldStop()
        mq.cmd('/popup Lets start our Bar Run!')
        -- Linear bar leg: (1) move + invis for cities → (2) West FP painting (Tassel) → (3) East FP (Grub/Betty). No extra invis between (2) and (3).
        pppokerEnsureInvisForCityEntry("West Freeport to East Freeport bar leg")

        -- (2) Tassel's Tavern painting — West Freeport (Poker2: nav + wait; then continue — no task hard-stop).
        runTasselsPaintingWaypoint()

        if not waitUntilTasselObjectiveCredited(120000) then
            gui.lastRunGainedCoins = 0
            unpauseRGMercs()
            unpauseCWTNPlugins()
            return
        end

        -- (3) Grub n Grog + Bluffing Betty — East Freeport.
        -- No second invis prep here: bar-leg invis was already applied above; re-calling trips
        -- group AA / spell fallback on shared timers after Tassel nav.
        ...
        elseif needMid2 <= 2 or stillEast then
            -- East Freeport. Crab and Grog Tavern. Spawn Bluffing Betty.
            mq.cmd('/squelch /nav locyxz 153 -806 7')
            moving()
            mq.delay(1500)
            local bettyInRange = waitForNPCToBeWithinDistance(PP.NPC_BLUFFING_BETTY, 25, 12000)
            if not bettyInRange then
                pppokerGuiDebug("Betty not in range yet — re-naving to tavern pocket.")
                mq.cmd('/squelch /nav locyxz 153 -806 7')
                moving()
                mq.delay(1200)
                bettyInRange = waitForNPCToBeWithinDistance(PP.NPC_BLUFFING_BETTY, 25, 20000)
            end
            if not bettyInRange then
            ...
            targetOrFail(PP.NPC_BLUFFING_BETTY, "Could not target Bluffing Betty")
            mq.delay(1000)
            mq.cmd('/face fast')
            ...
            -- Charm item gate.
            local slideUsable, slideReq, myLevel = canUseZueriaSlideNow()
            local slideReady = nil
            if slideUsable then
                slideReady = ensureZueriaMode(PP.ZUERIA_TARGET_MODE)
            else
                pppokerGuiDebug(string.format(
                    "Zueria Slide unavailable at level %s (requires %s) — skipping slide and using Gate/travel fallback.",
                    tostring(myLevel or "?"),
                    tostring(slideReq or "?")
                ))
            end
            if slideUsable and slideReady and slideReady:find(PP.ZUERIA_TARGET_MODE, 1, true) then
                mq.cmdf('/useitem "%s"', slideReady)
                -- Use a soft wait: zoning(25) calls fail() and schedules a restart.
                -- If the toon can't use the slide (level/cooldown), we want to continue via stage 3.
                info("Zueria Slide: waiting for cast/zoning (~22s)…")
                mq.delay(22000)
                local zoned = false
                for attempt = 1, 2 do
                    ...
                    mq.cmdf('/useitem "%s"', slideReady)
                    zoned = waitForZoneOrFalse(25, 30000)
                    if zoned then break end
                end
                if not zoned then
                    pppokerGuiDebug("Zueria slide did not zone to 25 in time — continuing without slide (stage 3 will handle).")
                end
            end
            mq.delay(1000)
        end
    else
        info(string.format("Skipping stage 2/8 (East Freeport): task past this block and Tassel's complete (at stage %s).", tostring(needAt2)))
        setGuiStage(needAt2 or 2, string.format("Skipped — route ahead of block 2 (~%s)", tostring(needAt2)))
    end
end
...
if startStage <= 3 then
    ...
    if needs3 then
        setGuiStage(3, "Neriak Foreign Quarter (Bull Pit / Svunsa)")
        shouldStop()
        mq.delay(1000)
        mountIfNeeded()
        mq.delay(3000)
        ...
        setGuiStage(needAt3 or 3, string.format("Skipped — route ahead of block 3 (~%s)", tostring(needAt3)))
        leaveNeriakTowardHubIfNeeded()
    end
end
...
if startStage <= 4 then
    ...
    if needs4 then
        setGuiStage(4, "Neriak Commons (Blind Fish / Toadstool)")
        shouldStop()
        ...
        setGuiStage(needAt4 or 4, string.format("Skipped — route ahead of block 4 (~%s)", tostring(needAt4)))
        leaveNeriakTowardHubIfNeeded()
    end
end
...
if startStage <= 5 then
    ...
    if needs5 then
        setGuiStage(5, "Highpass Hold (Quinn of Quads)")
        shouldStop()
        ...
        mq.delay(1500)
        targetOrFail(PP.NPC_QUINN_OF_QUADS, "Could not target Quinn of Quads")
        mq.delay(1000)
        mq.cmd('/keypress hail')
        mq.delay(1500)
        mq.cmd('/target ${Me.Name}')
        ...
        moving()
        mq.delay(1000)
        targetOrFail(PP.NPC_MHRAI_QUEEN_OF_TAILS, "Could not target Mhrai, Queen of Tails")
        mq.delay(1000)
        mq.cmd('/keypress hail')
        mq.delay(1500)
        mq.cmd('/target ${Me.Name}')
        ...
        setGuiStage(needAt5 or 5, string.format("Skipped — route ahead of block 5 (~%s)", tostring(needAt5)))
    end
end
...
if startStage <= 6 then
    ...
    if needs6 then
        ...
        setGuiStage(needAt6 or 6, string.format("Skipped — route ahead of block 6 (~%s)", tostring(needAt6)))
    end
end
...
if startStage <= 7 then
    ...
    if needs7 then
        ...
        setGuiStage(needAt7 or 7, string.format("Skipped — route ahead of block 7 (~%s)", tostring(needAt7)))
    end
end
...
if startStage <= 8 then
    ...
    if needs8 then
        setGuiStage(8, "West Freeport — hail Big Slick (reward)")
        shouldStop()
        mq.delay(1000)
        -- Final hail at Slick — same safe pocket; no city invis needed here.
        if mq.TLO.Zone.ID() ~= 383 then
        ...
        -- Do not hail Slick as "done" if journal still has incomplete rows (e.g. Tassel's skipped when resume jumped past stage 2).
        ...
        targetOrFail(PP.NPC_BIG_SLICK_JONES, "Could not target Big Slick Jones (reward)", 15000)
        mq.delay(1000)
        mq.cmd('/face fast')
        mq.cmd('/keypress hail')
        mq.delay(1000)
        ...
        setGuiStage(needAt8 or 8, string.format("Skipped — route ahead of block 8 (~%s)", tostring(needAt8)))
    end
end
-- Completion beeps.
mq.cmd('/beep')
